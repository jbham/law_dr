...
const defaultGetLabel = (date, index) => {
  var sdf = new Date(date); //.toDateString().substring(4);
  return `${sdf.getUTCFullYear()}-${sdf.getUTCMonth() + 1}-${sdf.getUTCDate()}`;
};
...
class HorizontalTimeline extends React.Component {
  render() {
    ...
    const dates = props.values.map((value) => new Date(value + "Z"));
